  ...
  try {
    ...
    if (user) {
      if (itemType === 'book') {
        ...
        user.favorites.connections = user.favorites.connections.filter (
          id => id.toString () !== itemId
        );
      }
...
router.post ('/', upload.single ('avatar'), async (req, res) => {
  ...
  // Update the GET favorites route in userRoutes.js with better error handling
  router.get ('/:id/favorites', async (req, res) => {
    try {
      console.log (`Fetching favorites for user ID: ${req.params.id}`);

      const user = await User.findById (req.params.id)
        .populate ('favorites.books')
        .populate ('favorites.movies')
        .populate ({
          path: 'favorites.connections',
          populate: [{path: 'book'}, {path: 'movie'}],
        });
